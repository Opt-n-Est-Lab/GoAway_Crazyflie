...
SPEED_FACTOR = 0.3
# Define minimum height for landing
MIN_HEIGHT = 0.1  # 0.1 meters (10 cm) for stable landing
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, URI):
        ...
        self.landing = False  # Track if landing has started
    ...
    def updateHover(self, k, v):
        if k != 'height':
            ...
            if not self.landing:  # Only adjust height if not landing
                self.hover[k] += v

            # Check if height is below the minimum threshold for landing
            if self.hover['height'] <= MIN_HEIGHT:
                self.smooth_landing()
    ...
    def smooth_landing(self):
        if not self.landing:
            ...
            self.landing = True
            while self.hover['height'] > 0:
                self.hover['height'] -= 0.05  # Gradually reduce height
                self.sendHoverCommand()
                time.sleep(0.1)
            self.cf.commander.send_stop_setpoint()  # Stop all movement
            print("Landing complete. Exiting program.")
            self.cf.close_link()  # Disconnect
